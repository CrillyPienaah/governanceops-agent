...
class PersistentAuditLog(AuditLog):
    ...
    The in-memory append and the disk write are serialized together
    under one lock here, separate from AuditLog's own internal lock.
    Without that, two concurrent append() calls could get correctly
    ordered sequence numbers from the in-memory chain (AuditLog's own
    lock guarantees that part) but then race on the file write itself
    -- landing in the file in a different order than their sequence
    numbers imply. Nothing gets corrupted in memory, but the on-disk
    log would no longer read back in the order the in-memory chain
    actually assigned, and a reload would either fail to verify a
    perfectly fine log or silently misorder entries.
    """
